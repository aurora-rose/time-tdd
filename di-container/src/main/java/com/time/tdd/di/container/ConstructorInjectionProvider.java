...
class ConstructorInjectionProvider<T> implements ContextConfig.ComponentProvider<T> {
    ...
    private static <T> List<Method> getInjectMethods(Class<T> component) {
        ...
        while (current != Object.class) {
            injectMethods.addAll(stream(current.getDeclaredMethods())
                ...
                .filter(m -> stream(component.getDeclaredMethods()).filter(cm -> !cm.isAnnotationPresent(Inject.class))
    ...
    public List<Class<?>> getDependencies() {
        return Stream.concat(Stream.concat(stream(injectConstructor.getParameters())
                .map(Parameter::getType), injectFields.stream().map(Field::getType)),
